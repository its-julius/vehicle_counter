            ...
            if det is not None and len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if names[int(cls)] in vehicles:
                        ...
                        x_temp = (matrix[0, 0] * cp[0] + matrix[0, 1] * cp[1] + matrix[0, 2]) / (
                                    matrix[2, 0] * cp[0] + matrix[2, 1] * cp[1] + matrix[2, 2])
                        y_temp = (matrix[1, 0] * cp[0] + matrix[1, 1] * cp[1] + matrix[1, 2]) / (
                                    matrix[2, 0] * cp[0] + matrix[2, 1] * cp[1] + matrix[2, 2])
                        # Plot the center point based
                        cv2.circle(map_2d, (int(x_temp), int(y_temp)), 5, vehicle_color[names[int(cls)]], 5)
